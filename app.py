...
def queryDB(queryCols):
    ...
    for queryCol in queryColElems:
        if has_or_rule and (queryCol == 'indoor' or queryCol == 'outdoor'):
            continue
    ...
    with f'Server=tcp:my-usc-server.database.windows.net,1433;Initial Catalog=my-database;Persist Security Info=False;User ID=adminserver;Password={password};MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;' as conn:
        with conn.cursor() as cursor:
            ...
            queryResult = [dict(zip([column[0] for column in cursor.description], row))
